...
1. On Linux, create the managed `/srv` layout with the allowlisted helper when available: `sudo -n rakkib privileged ensure-layout --state .fss-state.yaml`. This creates `{{DATA_ROOT}}`, `{{DATA_ROOT}}/docker`, `{{DATA_ROOT}}/data`, `{{DATA_ROOT}}/apps/static`, `{{DATA_ROOT}}/backups`, and `{{DATA_ROOT}}/MDs` without running the installer as root. If `sudo -n` fails because authorization expired, stop and ask the user to run `rakkib auth sudo` in a terminal.
