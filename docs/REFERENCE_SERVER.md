...
**Last Updated**: 2026-04-09 | **Server**: Rakkib reference server | **OS User**: `rakkib` | **IP**: `192.168.0.235` | **Domain**: `example.com` | **TZ**: `Asia/Riyadh`
...
|-----------|--------|------|
| `root-site` | `example.com` | Landing page |
| `form` | `form.example.com` | React SPA |
| `meddare` | `meddare.example.com` | React SPA |
| `dashboards-html` | `dashboards.example.com` | Static HTML |
...
- Host services accessed via `172.18.0.1`; SSH: local `192.168.0.235` or remote `ssh.example.com`
...
## Backup Strategy
Manual cloud-only to Google Drive (`backup@example.com` → `rakkibBackup/`).
Covers: `/home/rakkib`, `/srv/docker/`, Postgres dump, `/srv/data/`, OpenClaw config.
...
`/home/rakkib/.local/bin/status` — server status summary
