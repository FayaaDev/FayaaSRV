...
1. Run `./scripts/fayaasrv-doctor --json` from the repo root.
2. Read the JSON report and stop on any check with `"status":"fail"`.
3. Treat blocking failures as install gates that must be fixed before continuing. Blocking checks include OS support, Docker daemon access, Docker Compose v2, and ports 80/443 being free or owned by FayaaSRV Caddy.
...
5. Use the same command as a standalone diagnostic any time the user asks whether the host is healthy enough for FayaaSRV.
