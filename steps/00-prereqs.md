...
5. If Docker is not installed, install Docker by platform:
   ...
   - Linux: on Ubuntu, call `/usr/local/libexec/fayaasrv-root-helper install-docker-engine-ubuntu --admin-user {{ADMIN_USER}}`. This reviewed path also installs `acl` so the helper can use `setfacl` for the same-session Docker socket bridge.
