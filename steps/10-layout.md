...
1. On Linux, create the managed `/srv` layout with the allowlisted helper when available: `sudo ./bin/rakkib privileged ensure-layout --state .fss-state.yaml`. This creates `{{DATA_ROOT}}`, `{{DATA_ROOT}}/docker`, `{{DATA_ROOT}}/data`, `{{DATA_ROOT}}/apps/static`, `{{DATA_ROOT}}/backups`, and `{{DATA_ROOT}}/MDs` without running the full agent as root.
