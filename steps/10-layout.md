...
1. On Linux, create the managed `/srv` layout through the helper instead of creating root-owned paths directly from the unprivileged installer:
   `/usr/local/libexec/rakkib-root-helper ensure-linux-layout --admin-user {{ADMIN_USER}} [--with-service caddy] [--with-service cloudflared] [--with-service postgres] [--with-service nocodb] [--with-service n8n] [--with-service dbhub] [--with-service immich] [--with-service openclaw]`
